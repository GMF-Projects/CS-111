...
const unsigned long STEP_DELAY_US = 2900;
...
void loop() {
  ...
  if (cmdM45 == 'G' || cmdM45 == 'H' || cmdM45 == 'I' || cmdM45 == 'J') {
    ...
    if (cmdM45 == 'G') {          // Pitch forward
      digitalWrite(dirPinM4, HIGH);
      digitalWrite(dirPinM5, HIGH);
    }
    else if (cmdM45 == 'H') {     // Pitch reverse
      digitalWrite(dirPinM4, LOW);
      digitalWrite(dirPinM5, LOW);
    }
    else if (cmdM45 == 'I') {     // Roll left
      digitalWrite(dirPinM4, HIGH);
      digitalWrite(dirPinM5, LOW);
    }
    else if (cmdM45 == 'J') {     // Roll right
      digitalWrite(dirPinM4, LOW);
      digitalWrite(dirPinM5, HIGH);
    }
    ...
    if (nowMicros - lastStepTime >= STEP_DELAY_US) {
      ...
      digitalWrite(stepPinM5, stepStateM4);
    }
  ...
  // Disable M4/M5/M6 after inactivity
